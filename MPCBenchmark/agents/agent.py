...
class Agent:
    ...
    def predict_action(self, state, g_z=None, goal_state=None):
        ...
        if self.logging:
            self.planned_xs, self.planned_costs = self.simulate_trajectory(
                state, self.planned_us, g_z)
            self._log_iteration(
                self.planned_xs, self.planned_us, self.planned_costs)
    ...
    def _log_iteration(self, planned_xs, planned_us, planned_costs):
        ...
        self.planning_state_history.append(
            (self.step_iteration_variable, planned_xs.copy()))
        self.planning_action_history.append(
            (self.step_iteration_variable, planned_us.copy()))
    ...
    def simulate_trajectory(self, x, us, g_z):
        ...
        for i in range(1, self.horizon_length+1):
            newstate = self.model.predict(
                xs[i-1, :], us[i-1, :], goal=g_z[i-1, :])
